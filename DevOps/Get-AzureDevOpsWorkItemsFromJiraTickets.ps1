...
.DESCRIPTION
The Get-AzureDevOpsWorkItemsFromJiraTickets function connects to Azure DevOps via API, makes a query for work items associated with each provided Jira ticket, and returns the associated Azure DevOps work item(s) if they exist. If a work item is also associated with an Azure DevOps pull request, that association is also returned.
...
$AzureDevOpsPullRequests = @({id="1", title="PR1"}, {id="2", title="PR2"})
Get-AzureDevOpsWorkItemsFromJiraTickets -JiraTickets $JiraTickets -AzureDevOpsProject $AzureDevOpsProject -AzureDevOpsToken $AzureDevOpsToken -AzureDevOpsPullRequests $AzureDevOpsPullRequests
```
...
#>
function Get-AzureDevOpsWorkItemsFromJiraTickets() {
    Param (
    ...
    foreach ($jiraTicket in $JiraTickets) {        
        ...
        if ($devopsQueryResponse.StatusCode -eq 200) {
            ...
            $devopsPullRequestWorkItem = $AzureDevOpsPullRequests | Where-Object { $_.id -eq $devopsWorkItem.Id } | Select-Object -First 1
...
Export-ModuleMember -Function Get-AzureDevOpsWorkItemsFromJiraTickets